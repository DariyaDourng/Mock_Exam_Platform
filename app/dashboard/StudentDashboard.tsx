...
interface RawExamData {
  ...
  exam?: {
    ...
    category_id: number; // This links to the category name (e.g., ID for "Mathematics")
...
// Interface for the raw data received from the backend API for categories
// This mirrors the structure of the data returned by your Laravel backend for categories
...
export default function StudentHistoryPage() {
  ...
  const [filterCategory, setFilterCategory] = useState("all"); // Stores selected category for filtering
  const [categories, setCategories] = useState<string[]>(["all"]); // Stores unique category names for filter dropdown
  ...
  // Stores a map of category IDs to names for efficient lookup during data processing
  const [allCategoriesMap, setAllCategoriesMap] = useState<
    Record<number, string>
  >({});

  // useEffect hook to fetch exam history data and categories when the component mounts
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true); // Set loading to true at the start of data fetching

        // 1. Fetch categories first to create a lookup map (ID to Name)
        // This is crucial to convert category_id from exam data into readable category names
        ...
        // Assuming your API returns category data in `response.data.data`
        const categoriesData: RawCategoryData[] = categoriesRes.data.data || [];
        // Create a map: category.id -> category.name
        ...
        setAllCategoriesMap(categoriesMap); // Store the generated map in state
        ...
        Object.values(attemptsGrouped).forEach((attempts) => {
          ...
          sortedAttempts.forEach((item, index) => {
            ...
            // Get the category name using the pre-built categories map
            const categoryName = item.exam?.category_id
              ? categoriesMap[item.exam.category_id] || "unknown" // Fallback if category ID not found in map
              : "unknown"; // Fallback if 'exam' object or 'category_id' is missing
        ...
        // Extract unique category names from the formatted data to populate the category filter dropdown
        const uniqueCategories = Array.from(
          new Set(formatted.map((e) => e.category)),
        );
        setCategories(["all", ...uniqueCategories]); // Add "all" option to the beginning
      } catch (error) {
  ...
  // Filter the exam history based on the current search term and selected category filter
  const filteredHistory = examHistory.filter((exam) => {
    // Check if exam title or category includes the search term (case-insensitive)
    const matchesSearch =
      exam.examTitle.toLowerCase().includes(searchTerm.toLowerCase()) ||
      exam.category.toLowerCase().includes(searchTerm.toLowerCase());
    // Check if the exam's category matches the selected filter, or if "all" categories are selected
    const matchesCategory =
      filterCategory === "all" || exam.category === filterCategory;
    // Return true only if both search and category filters match
  ...
  return (
    <div className="space-y-6 p-2 py-0 sm:p-6 lg:p-4 max-w-8xl mx-auto font-inter">
      ...
      {/* Filters Section: Contains search input and category filter dropdown */}
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            <Select value={filterCategory} onValueChange={setFilterCategory}>
              ...
              <SelectContent>
                {/* Map through unique categories to create dropdown items */}
                {categories.map((category) => (
                  <SelectItem key={category} value={category}>
                    {/* Display "All Categories" or capitalize the first letter of the category name */}
                    {category === "all"
                      ? "All Categories"
                  ...
                  {exam.status === "completed" && (
                    <>
                      ...
                      >
                        ...
                        <Link
                          href={`/dashboard/tests/${exam.examAttemptId}/results?score=${exam.rawScore}`}
                        >
