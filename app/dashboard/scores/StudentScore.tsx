...
export default function StudentScore() {
      ...
      ? [
          {
            ...
            category: testId === "1" ? "Math" : "Logic IQ",
            questions: testId === "1" ? 30 : 25,
            correct: Math.round(
              (Number.parseInt(score) / 100) * (testId === "1" ? 30 : 25),
            ),
          },
          ...testHistory,
        ]
  ...
  const filteredTests =
    categoryFilter === "all"
      ? allTests
      : allTests.filter((test) => test.category === categoryFilter);
  ...
  const mathTests = allTests.filter((test) => test.category === "Math");
  const logicTests = allTests.filter((test) => test.category === "Logic IQ");

  const mathAvg = mathTests.length
    ? mathTests.reduce((sum, test) => sum + test.score, 0) / mathTests.length
    : 0;

  const logicAvg = logicTests.length
    ? logicTests.reduce((sum, test) => sum + test.score, 0) / logicTests.length
    : 0;

  const overallAvg = allTests.length
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Track your test performance over time
            </p>
          </div>
        ...
        {testId && score && (
          <Card className="border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950">
            <CardHeader>
              ...
              <CardDescription>
                You've completed the test with a score of {score}%.
              </CardDescription>
            </CardHeader>
          </Card>
        )}
          ...
          <TabsContent value="list" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Your past test results and performance
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                    ...
                    filteredTests.map((test) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <Badge className={getScoreBadge(test.score)}>
                            {test.score}%
                          </Badge>
                        </div>
