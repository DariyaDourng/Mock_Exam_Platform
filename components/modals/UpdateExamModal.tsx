...
export function UpdateExamModal({
    ...
    async function fetchExam() {
      ...
      try {
        ...
        const mappedExam = {
          id: data.id,
          name: data.name || "",
          category: data.category_id?.toString() || "", // backend uses category_id
  ...
  if (loading && isOpen) {
    return (
      <div className="p-4 text-sm text-gray-500">Loading exam data...</div>
    );
  }
  ...
  const handleSubmit = async (updatedExam: any) => {
    const payload = {
      ...
      category_id: Number(updatedExam.category),
